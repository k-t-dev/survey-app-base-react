...
////  ~~~~~~~~~~~~~~~~~~~~ 店舗 ~~~~~~~~~~~~~~~~~~~~
  ...
  const handleAddShop = async () => {
    ...
    try {
      ...
      const link_response = await fetch(`http://0.0.0.0:5001/survey-link/${relating_company_id}/${shop_id_for_link}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        }
                    ...
                    {companyShops.length > 0 ? (
                      companyShops.map((shop, shopIndex) => (
                        <tr key={shop.id}>
                          ...
                          <td><a href={shop.google_url} target="_blank" rel="noopener noreferrer">{shop.google_url}</a></td>
                          <td>
